  ...
  const totalPrice = () => {
    ...
    cartItems.map((item) => {
      ...
      if (product) {
        prices.push({
          price:
            products.filter((product) => product.id == item.id)[0].price || 0,
          count: item.count,
        });
      }
    });
    let sum = prices.reduce(function (acc, item) {
                    ...
                    <Fragment key={product.id}>
                      <div className="d-flex justify-content-between bg-light px-2 py-3 border mb-1">
                        ...
                        <div className="col-md-4 text-end">
                          ...
                          <span>{cartItems[index].count}</span>
                          <Button
